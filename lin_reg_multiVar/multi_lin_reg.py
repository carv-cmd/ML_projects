...
def gradient_descent(L, Epoch):
    ...
    for irx in range(Epoch):
        ...
        #           f'\ntheta_initials = {t}'
        #           f'\n-----------------------')
        x4_grad = (1/n) * x4 * (((x0 * t[0]) + (x1 * t[1]) + (x2 * t[2]) + (x3 * t[3]) - (x4 * t[4])) - Y)
        x3_grad = (1/n) * x3 * (((x0 * t[0]) + (x1 * t[1]) + (x2 * t[2]) + (x3 * t[3]) - (x4 * t[4])) - Y)
        x2_grad = (1/n) * x2 * (((x0 * t[0]) + (x1 * t[1]) + (x2 * t[2]) + (x3 * t[3]) - (x4 * t[4])) - Y)
        x1_grad = (1/n) * x1 * (((x0 * t[0]) + (x1 * t[1]) + (x2 * t[2]) + (x3 * t[3]) - (x4 * t[4])) - Y)
        x0_grad = (1/n) * (((x0 * t[0]) + (x1 * t[1]) + (x2 * t[2]) + (x3 * t[3]) - (x4 * t[4])) - Y)
